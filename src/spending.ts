...
export class SpendingTracker {
  ...
  constructor(config: BudgetConfig, symbol = 'CSPR') {
    this.config = config
    this.symbol = symbol
  }

  check(amountStr: string): void {
    ...
    if (amount > maxPerCall) {
      throw new Error(
        `Amount ${amountStr} ${this.symbol} exceeds per-call limit of ${this.config.maxPerCall} ${this.symbol}`
      )
    }

    if (this.spentToday + amount > maxPerDay) {
      throw new Error(
        `Payment would exceed daily limit of ${this.config.maxPerDay} ${this.symbol} (spent today: ${this.spentToday.toFixed(4)} ${this.symbol})`
      )
    }
  }

  record(amountStr: string, recipient: string, network: string): void {
    const amount = parseFloat(amountStr)
